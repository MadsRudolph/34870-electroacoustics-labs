...
\graphicspath{{../figures/}{Lab A/figures/}}  % local build runs in report/, Overleaf may run from the project root
...
\draw (0,0) node[ground]{} to[isource, l=$F$] (0,3) -- (2,3) node[circ]{} node[above]{$u_{vc}$};
\draw (2,3) to[C, l=$M_{mvc}$] (2,0) node[ground]{};
\draw (4,3) to[L, l=$C_{msp}$] (4,0) node[ground]{};
\draw (6,3) to[R, l=$1/R_{msp}$] (6,0) node[ground]{};
\draw (2,3) -- (6,3);
\draw (6,3) -- (8,3) to[L, l_=$C_{md}$] (10.5,3) -- (11.5,3) node[circ]{} node[above]{$u_d$};
\draw (8,3) -- (8,4.4) to[R, l=$1/R_{md}$] (10.5,4.4) -- (10.5,3);
\draw (11.5,3) to[C, l=$M_{md}$] (11.5,0) node[ground]{};
\draw (13.5,3) to[L, l=$C_{msr}$] (13.5,0) node[ground]{};
\draw (15.5,3) to[R, l=$1/R_{msr}$] (15.5,0) node[ground]{};
\draw (11.5,3) -- (15.5,3);
\end{circuitikz}
...
10 Hz & --- & \num{-123.0} \\
dip & \SI{42.2}{\hertz} & \num{-132.0} \\
\textbf{peak} & \SI{77.1}{\hertz} & \num{-98.9} \\
dip & \SI{142.9}{\hertz} & \num{-143.9} \\
\textbf{peak} & \SI{179.9}{\hertz} & \num{-112.3} \\
dip & \SI{309.7}{\hertz} & \num{-170.3} \\
\textbf{peak} & \SI{356.5}{\hertz} & \num{-141.6} \\
1 kHz & --- & \num{-253.9} \\
\bottomrule\end{tabular}
...
hence $|Z_{in}|\approx\num{1.5e6}$ and \SI{-123}{\decibel}. The three peaks (77.1, 179.9, \SI{356.5}{\hertz}) are the
...
two poles, close to the $Z_{in}$ maxima (47, 176, \SI{191}{\hertz}) and to the $M_{A1}$--$C_{A2}$ pair
(\SI{310}{\hertz}). Above the last resonance the response falls at about \SI{18}{\decibel} per octave per chamber section
(a ladder of three L--C low-pass sections), from \SI{-142}{\decibel} at \SI{357}{\hertz} to \SI{-254}{\decibel} at
\SI{1}{\kilo\hertz}. The losses $R_A$ only round off the peaks: at \SI{100}{\hertz} $\omega M_{A3} = \num{4.7e6}\gg\num{2.5e4}$,
so the Q-factors are in the hundreds and the peaks are very sharp --- sharp enough that the sweep needs about 2000 points per decade to resolve their height (200 points per decade reads the \SI{77}{\hertz} peak \SI{2}{\decibel} and the \SI{357}{\hertz} peak \SI{8}{\decibel} too low).

\begin{figure}[!htb]\centering
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{part2a_pressure_source_Uout}\caption{pressure source, \SI{1}{\pascal}}\label{fig:p2a}\end{subfigure}
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{part2b_volume_velocity_source_Uout}\caption{volume-velocity source, \SI{1}{\cubic\metre\per\second}}\label{fig:p2b}\end{subfigure}
...
10 Hz & --- & $+0.45$ ($\to 0$ as $f\to 0$) \\
\textbf{peak} & \SI{47.1}{\hertz} & $+38.6$ \\
dip & \SI{112.1}{\hertz} & $-5.2$ \\
\textbf{peak} & \SI{175.6}{\hertz} & $+40.9$ \\
dip & \SI{183.7}{\hertz} & $+20.1$ \\
\textbf{peak} & \SI{190.8}{\hertz} & $+41.1$ \\
1 kHz & --- & $-111.4$ \\
\bottomrule\end{tabular}
\caption{Part 2b: transfer $U_{out}/U_{in}$ for the volume-velocity source.}
\end{table}
Transmission loss $\mathrm{TL} = -20\log|U_{out}/U_{in}|$: \SI{-1.9}{\decibel} at \SI{20}{\hertz}, \SI{-19.3}{\decibel}
at \SI{50}{\hertz}, $+4.7$ at 100, $-0.5$ at 150, $-6.1$ at 200, $+40.9$ at 300, $+73.4$ at 500, $+92.2$ at 700 and
$+\SI{111.4}{\decibel}$ at \SI{1}{\kilo\hertz}.
...
peaks sit at the open-inlet natural frequencies, the \emph{maxima} of $Z_{in}$ (47.1, 175.6 and \SI{190.8}{\hertz}) ---
...
\emph{amplifies} the flow by about \SI{40}{\decibel}, limited only by the pipe losses. A real engine exhaust is somewhere between the two ideal sources
...
per octave per section, reaching \SI{111}{\decibel} of transmission loss at \SI{1}{\kilo\hertz}. The second and third
open-inlet modes are close neighbours (176 and \SI{191}{\hertz}) separated by a $+\SI{20}{\decibel}$ saddle.

\emph{A modelling trap worth recording.} The ideal current source must not be loaded. At these anti-resonances
$|Z_{in}|$ reaches \SI{2.9e8}{\pascal\second\per\cubic\metre} at \SI{47}{\hertz} and \SI{1.5e9}{} at \SI{191}{\hertz}, so a
\SI{100}{\mega\ohm} ``DC-leak'' resistor across the source --- a common SPICE habit --- takes most of the source flow exactly
at the peaks: it lowers them to $+25.8$ and $+\SI{26.6}{\decibel}$ and merges the upper two into one hump. The pipe
inductors already provide the DC path, so no leak is needed (the sheets use \SI{1}{\tera\ohm}); LTspice with an
unloaded source gives the same three peaks.

\begin{figure}[!htb]\centering
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{part2ab_source_comparison}\caption{$U_{out}$ for both sources, normalised}\label{fig:p2ab}\end{subfigure}
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{part2_input_impedance}\caption{input impedance $Z_{in} = p_{in}/U_{in}$ of the ladder}\label{fig:p2zin}\end{subfigure}
...
3 & \SI{-30.2}{\decibel} & $+21.0$ dB at 47 Hz, $+34.6$ at 176 Hz, $+45.6$ at 191 Hz; dip \SI{-22}{\decibel} at \SI{51}{\hertz} \\
5 & \SI{-78.6}{\decibel} & $+37.7$ at 47 Hz, $+32.7$ at 176 Hz, $+37.2$ at 191 Hz; dip \SI{-43}{\decibel} at \SI{149}{\hertz} \\
7 & \SI{-111.4}{\decibel} & $+38.6$ at 47 Hz, $+40.9$ at 176 Hz, $+41.1$ at 191 Hz \\
\bottomrule\end{tabular}
...
contributes 30--\SI{40}{\decibel} there. The dips in $U_3$ (\SI{51}{\hertz}) and $U_5$ (\SI{149}{\hertz}) are
...
same peaks at 47, 176 and \SI{191}{\hertz}. Inserting the radiation load changes $U_{out}$ by less than \SI{0.01}{\decibel}:
...
\begin{figure}[!htb]\centering
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{part2d_near_field_pressure}\caption{near field, $p_{open} = V(p_{out})$}\label{fig:p2d}\end{subfigure}
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{part2e_pressure_10m}\caption{far field, \SI{10}{\metre}, free space}\label{fig:p2e}\end{subfigure}
...
\SI{20}{\hertz} & 97.4 & 95.4 \\
\SI{50}{\hertz} & 122.6 & 103.4 \\
\SI{100}{\hertz} & 104.7 & 109.4 \\
\SI{200}{\hertz} & 121.5 & 115.4 \\
\SI{500}{\hertz} & 50.0 & 123.4 \\
...
radiated SPL is the $U_{out}$ transfer of b) plus that tilt. The silencer helps only above about \SI{205}{\hertz}
(roughly \SI{73}{\decibel} at \SI{500}{\hertz}, \SI{111}{\decibel} at \SI{1}{\kilo\hertz}) and \emph{hurts} by up to
about \SI{41}{\decibel} at its three resonances when driven by an ideal volume-velocity source; the finite source
impedance of a real engine is what tames those peaks.
...
% mechanical node
\draw (0,0) node[ground]{} to[isource, l=$F$] (0,3) -- (2,3) node[circ]{} node[above]{$u$};
\draw (2,3) to[generic, l=$Y_M$ (Part 1)] (2,0) node[ground]{};
% force sink
\draw (5.5,3) to[cI, l=$S\,p$] (5.5,0) node[ground]{};
\draw (2,3) -- (5.5,3);
% acoustic side
\draw (9.5,0) node[ground]{} to[cI, l=$S\,u$] (9.5,3) -- (11.5,3) node[circ]{} node[above]{$p$};
\draw (11.5,3) to[generic, l=$2\Zrad$] (11.5,0) node[ground]{};
\end{circuitikz}
...
\begin{figure}[H]\centering
\begin{circuitikz}[american,scale=0.78,every node/.style={transform shape}]
\draw (0,0) node[ground]{} to[V, l=$v$] (0,3) to[R, l=$R_c$, i=$i$] (2.5,3) to[L, l=$L_e$] (4.5,3)
   to[cV, l=$Bl\,u_c$] (4.5,0) node[ground]{};
\draw (7.7,0) node[ground]{} to[cI, l=$Bl\,i$] (7.7,3) -- (9.2,3) node[circ]{} node[above]{$u_c$};
\draw (9.2,3) to[C, l=$M_{mc}$] (9.2,0) node[ground]{};
\draw (9.2,3) -- (11,3) to[L, l_=$C_{ms}$] (13,3) -- (14,3) node[circ]{} node[above]{$u_1$};
\draw (11,3) -- (11,4.4) to[R, l=$1/R_{ms}$] (13,4.4) -- (13,3);
\draw (14,3) to[C, l=$M_{m1}$] (14,0) node[ground]{};
\draw (16,3) to[L, l=$C_{ms2}$] (16,0) node[ground]{};
\draw (18,3) to[R, l=$1/R_{ms2}$] (18,0) node[ground]{};
\draw (14,3) -- (18,3);
\end{circuitikz}
...
\begin{figure}[!htb]\centering
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{part4b_mechanical_impedance}\caption{$Z_M = Bl\,i/u_c$}\label{fig:p4b}\end{subfigure}
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{part4c_electrical_impedance}\caption{$Z_E = 1/I(V_{s1})$}\label{fig:p4c}\end{subfigure}
...
cone radiates; the silencer amplifying the exhaust by about \SI{40}{\decibel} at its resonances while attenuating
...
capacitor-only branch gets a leak resistor --- chosen far above the largest impedance in the circuit (\SI{1}{\tera\ohm}
for the silencer, whose input impedance reaches \SI{1.5}{\giga\ohm}), or it becomes part of the physics (Part~2b).
